...
def intLoss(img):
    ...
    one = torch.ones_like(down, requires_grad=True)
    zero = torch.zeros_like(down, requires_grad=True)
    img1 = torch.where(down > 0.5, one, zero)
    img2 = torch.nn.UpsamplingBilinear2d(scale_factor=8)(img1)
    # img2 = torch.nn.UpsamplingNearest2d(scale_factor=8)(img1)
    return img2
...
def dice_loss(y_true, y_pred, smooth = 0.001):
    ...
    dice_coeff = torch.abs((2. * intersection + smooth ) / (torch.sum(y_pred) + torch.sum(y_true) + smooth))
...
def Fusion_loss(vi, ir, fu, weights=[10, 10], device='cuda' if torch.cuda.is_available() else 'cpu'):
    ...
    # loss_intensity = F.l1_loss(fu, vi) + F.l1_loss(fu, ir)

    # loss_ssim = final_ssim(img_vis= vi, img_ir=ir, img_fuse= fu)

    loss_total = 5 * loss_grad + 1 * loss_intensity 
